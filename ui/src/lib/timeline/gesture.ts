...
export function start(
  ...
  const ids = selected.has(grabbed.clip) ? new Set(selected) : new Set([grabbed.clip])
...
export function carry(d: Drag, at: At, playhead: number, view: Viewport): Drag {
  const tracks = d.edge ? 0 : clamp((at.lane ?? d.held.track) - d.held.track, d.limits.tracks)
...
export function moveLimits(timeline: TimelineDto | null, group: Placement[]): Limits {
  const tracks = group.map((clip) => clip.track)

  return {
    frames: [-Math.min(...group.map((clip) => clip.position)), Infinity],
    tracks: [-Math.min(...tracks), (timeline?.tracks.length ?? 0) - Math.max(...tracks)],
  }
}

/** The tightest of what each clip allows: the engine refuses the group. */
function trimLimits(timeline: TimelineDto, group: Placement[], edge: Edge): Limits {
  let low = -Infinity
  let high = Infinity
