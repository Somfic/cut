<script lang="ts">
  ...
  function draw() {
    ...
    for (let frame = Math.floor(scroll / step) * step; xOf(frame) < width; frame += step) {
  ...
  function onWheel(event: WheelEvent) {
    ...
    if (event.ctrlKey || event.metaKey) {
      ...
      zoom = Math.min(MAX_ZOOM, Math.max(MIN_ZOOM, zoom * Math.exp(-event.deltaY / 200)))
