...
fn source(seconds: u64) -> Arc<Source> {
    Arc::new(Source::new(
        PathBuf::from(format!("{seconds}s.mp4")),
...
fn appends_after_everything_already_there() {
    ...
    assert_eq!(timeline.timebase, Rational::new(30, 1));
}
...
fn a_rejected_edit_changes_nothing() {
    ...
    let empty = Arc::new(Source::new(
        PathBuf::from("silence.wav"),
